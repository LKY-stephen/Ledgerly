...
import { createWritableStorageDatabaseFromWeb, useWebDatabaseContext } from "../../storage/provider.web";
...
export function useBusinessLedgerReports(input: {
  ...
  useEffect(() => {
    ...
    loadBusinessLedgerReportSnapshot({
      ...
      sdk: new LedgerlySDK(createWritableStorageDatabaseFromWeb(database)),
    })
      .then((snapshot) => {
        if (isMounted) {
